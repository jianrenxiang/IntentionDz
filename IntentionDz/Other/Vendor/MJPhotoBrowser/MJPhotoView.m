...
- (void)longPress:(UILongPressGestureRecognizer *)longPressGest {
    ...
    NHCustomActionSheet *actionSheet = [NHCustomActionSheet actionSheetWithCancelTitle:@"取消" alertTitle:@"保存图片" SubTitles:@"保存图片到相册", nil];
    [actionSheet show];
    WeakSelf(weakSelf);
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        [_imageView setImageWithURL:_photo.url placeHolder:_photo.srcImageView.image progressHandle:^(CGFloat progress) {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                loading.progress = progress;
            });
        } finishHandle:^(BOOL finished, UIImage *image) {
